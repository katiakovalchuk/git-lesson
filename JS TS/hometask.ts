...
// Implement a function `getObjectKeys()` which returns more convenient result for any argument given, so that you
// don't need to cast it.
...
//Implement swap which receives 2 persons and returns them in the reverse order. The function itself is already there,
// actually. We just need to provide it with proper types. Also this function shouldn't necessarily be limited to just
...
//Define type PowerUser which should have all fields from both User and Admin (except for type), and also have type
// 'powerUser' without duplicating all the fields in the code.
interface User {
    type: 'user';
    name: string;
    age: number;
...
//Remove UsersApiResponse and AdminsApiResponse types and use generic type ApiResponse
// in order to specify API response formats for each of the functions.
...
//Create a function promisifyAll which accepts an object with functions and returns a new object where each of the
// function is promisified. rewrite api creation accordingly:
...
type CbAsyncFunction<T> = (cb: (res: ApiResponse<T>) => void) => void;
type PromiseAsyncFunction<T> = () => Promise<T>;

export function promisify<T>(func: CbAsyncFunction<T>): PromiseAsyncFunction<T> {
